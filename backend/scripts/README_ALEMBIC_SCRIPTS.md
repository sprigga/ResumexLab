# Alembic 自動化腳本使用指南

**專案**: ResumeXLab
**版本**: 1.0
**日期**: 2026-01-04
**作者**: Polo (林鴻全)

---

## 📋 概述

本目錄包含三個 Alembic 遷移自動化腳本，用於簡化資料庫遷移操作並處理常見問題。

### 腳本列表

| 腳本名稱 | 類型 | 功能描述 |
|---------|------|---------|
| `alembic_helper.py` | Python | 多功能助手，提供狀態查詢、遷移執行、問題修復等 |
| `alembic_create_migration.sh` | Bash | 自動化遷移創建流程，包含備份和兼容性檢查 |
| `alembic_diagnose.sh` | Bash | 問題診斷工具，自動檢測常見配置和環境問題 |

---

## 🚀 快速開始

### 1. alembic_helper.py - 主要助手腳本

**功能**：提供完整的 Alembic 操作功能

#### 可用指令

```bash
# 查看使用說明
python scripts/alembic_helper.py help

# 檢查當前狀態
python scripts/alembic_helper.py status

# 執行健康檢查
python scripts/alembic_helper.py check

# 備份資料庫
python scripts/alembic_helper.py backup

# 執行遷移（會自動備份）
python scripts/alembic_helper.py migrate

# 標記資料庫版本
python scripts/alembic_helper.py stamp

# 修復 SQLite ALTER COLUMN 問題
python scripts/alembic_helper.py fix-sqlite

# 回滾到上一個版本
python scripts/alembic_helper.py rollback
```

#### 使用範例

```bash
# 場景 1: 首次設定，資料庫已有表但未標記版本
python scripts/alembic_helper.py status
# 發現: "資料庫尚未標記版本"
python scripts/alembic_helper.py stamp
# 結果: 資料庫被標記為最新版本

# 場景 2: 執行新的遷移
python scripts/alembic_helper.py migrate
# 自動備份資料庫
# 執行遷移
# 顯示結果

# 場景 3: 遷移失敗，出現 ALTER COLUMN 錯誤
python scripts/alembic_helper.py fix-sqlite
# 自動掃描最新遷移檔案
# 註解掉不支援的操作
# 備份原檔案

# 場景 4: 需要回滾
python scripts/alembic_helper.py rollback
# 自動備份資料庫
# 回滾到上一版本
# 顯示當前版本
```

---

### 2. alembic_create_migration.sh - 遷移創建腳本

**功能**：自動化遷移創建流程

#### 基本用法

```bash
# 創建遷移（不自動應用）
./scripts/alembic_create_migration.sh "遷移描述"

# 創建並自動應用遷移
./scripts/alembic_create_migration.sh "遷移描述" --apply
```

#### 執行流程

此腳本會自動執行以下步驟：

1. **環境檢查**
   - 檢查虛擬環境
   - 檢查 Alembic 配置
   - 檢查資料庫存在性

2. **檢查當前版本**
   - 顯示當前資料庫版本
   - 警告未標記版本的情況

3. **備份資料庫**
   - 自動備份到 `backend/data/resume_backup_[timestamp].db`
   - 顯示備份大小

4. **生成遷移檔案**
   - 執行 `alembic revision --autogenerate`
   - 顯示生成的檔案路徑

5. **SQLite 兼容性檢查**
   - 自動偵測 `ALTER COLUMN` 操作
   - 提供自動修復選項
   - 顯示修復建議

6. **顯示變更摘要**
   - 統計新增/刪除的表和欄位
   - 警告沒有偵測到變更的情況

7. **應用遷移（可選）**
   - 使用 `--apply` 參數時自動執行
   - 顯示執行結果

#### 使用範例

```bash
# 範例 1: 新增欄位
./scripts/alembic_create_migration.sh "新增使用者頭像欄位"

# 範例 2: 新增表並自動應用
./scripts/alembic_create_migration.sh "新增技能表" --apply

# 範例 3: 修改現有結構
./scripts/alembic_create_migration.sh "更新工作經歷表結構"
```

#### 輸出範例

```
================================================
  Alembic 遷移創建助手
================================================

ℹ 遷移描述: 新增使用者頭像欄位

================================================
  步驟 1: 環境檢查
================================================

✓ 虛擬環境存在
✓ Alembic 配置存在
✓ 資料庫存在
ℹ 資料庫大小: 120K

================================================
  步驟 2: 檢查當前版本
================================================

✓ 當前版本: d711f173f9e3 (head)

================================================
  步驟 3: 備份資料庫
================================================

✓ 備份完成: data/resume_backup_20260104_173000.db
ℹ 備份大小: 120K

...
```

---

### 3. alembic_diagnose.sh - 診斷腳本

**功能**：快速診斷 Alembic 設定和常見問題

#### 基本用法

```bash
./scripts/alembic_diagnose.sh
```

#### 診斷項目

1. **虛擬環境檢查**
   - 檢查 .venv 存在
   - 檢查 Alembic 安裝
   - 顯示 Alembic 版本

2. **Alembic 配置檢查**
   - 檢查 alembic.ini
   - 檢查資料庫 URL 設定
   - 檢查 env.py 和 target_metadata

3. **資料庫狀態檢查**
   - 檢查資料庫檔案存在
   - 檢查 alembic_version 表
   - 列出所有資料表

4. **遷移一致性檢查**
   - 比對 Alembic HEAD 和當前版本
   - 檢查是否需要升級

5. **SQLite 兼容性檢查**
   - 掃描最新遷移檔案
   - 偵測 ALTER COLUMN 操作

6. **常見錯誤診斷**
   - 執行 `alembic check`
   - 檢查 Python cache

#### 輸出範例

```
================================================
  Alembic 問題診斷工具
================================================

ℹ 工作目錄: /path/to/backend

================================================
  檢查 1: 虛擬環境
================================================

✓ 虛擬環境存在
✓ Alembic 已安裝
ℹ 版本: alembic 1.12.0

================================================
  檢查 2: Alembic 配置
================================================

✓ alembic.ini 存在
✓ 資料庫 URL 已設定
ℹ URL: sqlite:///./data/resume.db
✓ alembic 目錄存在
✓ env.py 存在
✓ target_metadata 已設定
✓ versions 目錄存在 (2 個遷移)

...

================================================
  診斷總結
================================================

✓ 未發現問題！系統狀態良好
```

---

## 📖 常見使用場景

### 場景 1: 首次設定 Alembic

```bash
# 1. 檢查環境
./scripts/alembic_diagnose.sh

# 2. 如果資料庫已有表，標記版本
python scripts/alembic_helper.py stamp

# 3. 驗證狀態
python scripts/alembic_helper.py status
```

### 場景 2: 新增功能需要修改資料庫

```bash
# 1. 修改 SQLAlchemy Model
# 編輯 backend/app/models/*.py

# 2. 創建遷移
./scripts/alembic_create_migration.sh "新增功能XYZ的資料表"

# 3. 檢查生成的遷移檔案
# 如有 SQLite 兼容性問題，腳本會提示並提供修復選項

# 4. 應用遷移
python scripts/alembic_helper.py migrate
```

### 場景 3: 遷移失敗需要修復

```bash
# 1. 診斷問題
./scripts/alembic_diagnose.sh

# 2. 如果是 ALTER COLUMN 問題
python scripts/alembic_helper.py fix-sqlite

# 3. 重新執行遷移
python scripts/alembic_helper.py migrate
```

### 場景 4: 需要回滾變更

```bash
# 1. 查看當前狀態
python scripts/alembic_helper.py status

# 2. 回滾到上一版本
python scripts/alembic_helper.py rollback

# 3. 驗證結果
python scripts/alembic_helper.py status
```

### 場景 5: 定期維護檢查

```bash
# 每週執行一次健康檢查
python scripts/alembic_helper.py check

# 或使用診斷工具
./scripts/alembic_diagnose.sh
```

---

## 🔧 進階使用

### 自動修復 SQLite 兼容性問題

`alembic_helper.py` 的 `fix-sqlite` 指令會：

1. 掃描最新的遷移檔案
2. 找出所有 `op.alter_column` 操作
3. 自動註解這些不支援的操作
4. 備份原檔案為 `.py.bak`
5. 添加說明註解

**範例**：

原始遷移檔案：
```python
def upgrade() -> None:
    op.alter_column('project_details', 'id',
               existing_type=sa.INTEGER(),
               nullable=False,
               autoincrement=True)
    op.add_column('projects', sa.Column('attachment_name', sa.String(length=255)))
```

修復後：
```python
def upgrade() -> None:
    # 原本的 ALTER COLUMN 操作 (已註解於 2026-01-04，原因：SQLite 不支援 ALTER COLUMN)
    # op.alter_column('project_details', 'id',
    #            existing_type=sa.INTEGER(),
    #            nullable=False,
    #            autoincrement=True)
    op.add_column('projects', sa.Column('attachment_name', sa.String(length=255)))
```

### 批次處理多個遷移

```bash
#!/bin/bash
# 批次創建多個遷移

migrations=(
    "新增使用者頭像欄位"
    "新增技能表"
    "更新工作經歷索引"
)

for msg in "${migrations[@]}"; do
    echo "創建遷移: $msg"
    ./scripts/alembic_create_migration.sh "$msg"
    sleep 2
done

echo "所有遷移已創建，請檢查後執行應用"
```

---

## ⚠️ 注意事項

### 1. 備份重要性

所有涉及資料庫變更的腳本都會**自動備份**，但建議：

- 定期手動備份重要資料
- 備份檔案位於 `backend/data/resume_backup_*.db`
- 生產環境操作前**務必**額外備份

### 2. SQLite 限制

腳本會自動處理 SQLite 限制，但需了解：

- ✅ **支援**: ADD COLUMN, DROP COLUMN (SQLite 3.35.0+), RENAME TABLE
- ❌ **不支援**: ALTER COLUMN, DROP CONSTRAINT, 部分 ADD CONSTRAINT

### 3. 遷移檔案檢查

即使使用自動化腳本，仍建議：

- 檢查生成的遷移檔案內容
- 驗證 `upgrade()` 和 `downgrade()` 邏輯
- 確認變更符合預期

### 4. 環境需求

腳本需要：

- Python 3.10+
- Bash shell (macOS/Linux)
- SQLite3 指令（用於診斷）
- 虛擬環境 (.venv)

---

## 🐛 疑難排解

### 問題 1: 腳本無法執行

```bash
# 確認執行權限
ls -l scripts/alembic_*.sh

# 如果沒有執行權限，添加權限
chmod +x scripts/alembic_*.sh scripts/alembic_helper.py
```

### 問題 2: "找不到 alembic 指令"

```bash
# 確認虛擬環境
ls -la backend/.venv

# 重新安裝 Alembic
cd backend
source .venv/bin/activate
pip install alembic
```

### 問題 3: 診斷腳本顯示多個問題

```bash
# 依照診斷腳本的建議逐一修復
# 修復後再次執行診斷
./scripts/alembic_diagnose.sh
```

### 問題 4: 自動修復失敗

```bash
# 檢查遷移檔案備份
ls -la backend/alembic/versions/*.bak

# 手動恢復
cd backend/alembic/versions
mv xxxx.py.bak xxxx.py

# 手動編輯遷移檔案
```

---

## 📚 參考資源

- **專案文件**: `/docs/Alembic.md`
- **Alembic 官方文件**: https://alembic.sqlalchemy.org/
- **SQLAlchemy 文件**: https://www.sqlalchemy.org/

---

## 🔄 更新歷史

| 版本 | 日期 | 變更說明 |
|------|------|---------|
| 1.0 | 2026-01-04 | 初版，包含三個自動化腳本 |

---

## 👤 作者與維護

**作者**: Polo (林鴻全)
**Email**: sprigga@gmail.com
**專案**: ResumeXLab

---

## 📄 授權

這些腳本是 ResumeXLab 專案的一部分，僅供專案內部使用。

---

**最後更新**: 2026-01-04
